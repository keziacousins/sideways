...
export const POST: APIRoute = async ({ params, session }) => {
  ...
  while (true) {
    ...
    const check = await fetch(`${API_URL}/api/documents/${space}/${candidateSlug}`, {
  ...
  const res = await fetch(`${API_URL}/api/documents/${space}/${slug}`, {
    method: "PUT",
  ...
  return new Response(JSON.stringify({ slug, edit: true }), { status: 200, headers: { "Content-Type": "application/json" } });
};
