...
export const GET: APIRoute = async ({ params, session }) => {
  ...
  if (!token) return new Response(JSON.stringify({ watching: false }), { headers: { "Content-Type": "application/json" } });

  const res = await fetch(`${API_URL}/api/documents/${params.space}/${params.slug}/watch`, {
